...
module.exports.myGetAccountCheck = myGetAccountCheck;
...
Spreadsheet.load({
  debug: true,
  spreadsheetName: 'SuitApp',
  worksheetName: 'account',
  oauth2: {
    client_id:'1092863384945-s28bg9j0s03dgj98jnuhr68e7ga1sev3.apps.googleusercontent.com',
    client_secret:'Nt33gi-WmArrVO4RXw6ddgm3',
    refresh_token:'1/ZntQpuUg_oBkuqb0MYnYH5-7gURn1w3Vx1-A7CcRypA'
...
function myGetAccountCheck(opt, callback) {

  var error;
  var customizedInfo;
  var selectInfo = undefined;

  if (typeof opt !== 'object' || typeof opt.account !== 'string' || typeof opt.password !== 'string' ) {
    error = "opt wrong";
    console.log(typeof opt !== 'object' || typeof opt.account !== 'string' || typeof opt.password !== 'string' );
    return callback( error, selectInfo);
  }
  ...
  Spreadsheet.load({
    ...
    oauth2: {
      ...
      refresh_token:'1/ZntQpuUg_oBkuqb0MYnYH5-7gURn1w3Vx1-A7CcRypA'
        ...
        if ( err ) {
          console.log("test");
          return callback(err, selectInfo);
        }
        ...
        spreadsheet.receive( function(err, rows, info) {
          ...
          /* if recive is success.....*/

          /* re-structure data format.....*/
          customizedInfo = _customizeInfo(rows, info.totalRows);

          /* check account exist or not .....*/
          selectInfo = _getAccountCheck( opt, customizedInfo );

          callback(err, selectInfo);
...
function _customizeInfo(sheetObj, length) {
//--------------create the format of Json-------------------------
  var headers = sheetObj[1];                //form the key of obj
  var sheet_data = [];
...
function _getAccountCheck( _input, _sheetInfo){
  var result = _sheetInfo.filter(function(e){
    return e.account == _input.account &&
           e.password == _input.password;
  });
