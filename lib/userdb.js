...
var Custom = [];          //register for custom
  ...
  Spreadsheet.load({
    ...
    oauth2: db_oauth
      ...
      spreadsheet.receive(function(err,rows,info){
        ...
        if( WorksheetName == 'custom' ) {
            if( Custom.length == 0 )
              Custom = getSheetInfo( rows , info.totalRows );
            else if( Custom.length < info.totalRows-1 )
              Custom = getSheetInfo( rows , info.totalRows );
            else if( Custom.length > info.totalRows-1 ) {
              for( i = info.totalRows-1; i < Custom.length; i++) {
                input_obj = {};
                key_list = Object.keys(Custom[i]);
                for(j = 1; j <= key_list.length; j++)
                  term[j] = Custom[i][key_list[j-1]];
                input_obj[i+2] = term;
...
setInterval(function(){ refresh('custom');},180000);//3 minutes
